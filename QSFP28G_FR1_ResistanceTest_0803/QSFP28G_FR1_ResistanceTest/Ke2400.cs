...
namespace QSFP28G_FR1_ResistanceTest
{
    ...
    public class Ke2400 : Instrument
    {
        ...
        /// <param name="type">Voltage or current selection</param>
        public void setOutputLevel(float level, VIType type)
        {
            string wrt = string.Empty;
            try
            {
        ...
        public float ComplianceSetpoint { get; set; }


        //internal double Test()
        //{
        //    throw new NotImplementedException();
        //}
        public double VoltageTest()
        {
            string rtn = string.Empty;
            string command = "*RST";
            //this.gpib.Device.Write(command);
            //SetBeep(false);
            //command = ":SYSTem:BEEPer:STATe<0>";
            //this.gpib.Device.Write(command);
            command = ":SOUR:FUNC CURR";
            this.gpib.Device.Write(command);
            command = ":SOUR:CURR:MODE FIXED";
            //command = ":SOUR:VOLT 10";
            this.gpib.Device.Write(command);
            //command = ":SENS:VOLT:RANG 0.005";
            //this.gpib.Device.Write(command);
            command = ":SOUR:CURR:RANG 2E-6";
            this.gpib.Device.Write(command);
            command = ":SOUR:CURR:LEV 1E-6";
            this.gpib.Device.Write(command);
            command = ":SENS:VOLT:PROT 1";
            this.gpib.Device.Write(command);
            command = ":SENS:FUNC 'VOLT'";
            this.gpib.Device.Write(command);
            command = ":SENS:VOLT:RANG 1";
            this.gpib.Device.Write(command);
            command = ":FORM:ELEM VOLT";
            this.gpib.Device.Write(command);
            command = ":OUTP ON";
            this.gpib.Device.Write(command);
            command = ":READ?";
            //this.gpib.Device.Write(command);
            rtn = this.gpib.Query(command); //we can not use query()function 
            Thread.Sleep(100);
            //command = "/*:OUTP OFF*/ :OUTP OFF";
            //command = ":OUTP OFF";
            //this.gpib.Device.Write(command);
        ...
        //private double Scan(double points, double stopValue, double measRange)
        //{

        //    double step = stopValue / points;
        ...
        public string Scan()
        {
            ...
            string rtn = string.Empty;
                ...
                string command = "*RST";
                SetBeep(false);
                command = ":SENS:FUNC:CONC OFF";
                this.gpib.Device.Write(command);
                command = ":SOUR:FUNC CURR";
                this.gpib.Device.Write(command);
                command = ":SENS:FUNC 'VOLT:DC'";
                this.gpib.Device.Write(command);
                command = ":SENS:VOLT:PROT 2";
                this.gpib.Device.Write(command);
                command = ":SOUR:CURR:START 0";
                this.gpib.Device.Write(command);
                command = ":SOUR:CURR:STOP 80E-3";
                this.gpib.Device.Write(command);
                command = ":SOUR:CURR:STEP 5E-3";
                this.gpib.Device.Write(command);
                command = ":SOUR:CURR:MODE SWE";
                this.gpib.Device.Write(command);
                command = ":SOUR:SWE:RANG AUTO";
                this.gpib.Device.Write(command);
                command = ":SOUR:SWE:SPAC LIN";
                this.gpib.Device.Write(command);
                command = ":TRIG:COUN 16";
                this.gpib.Device.Write(command);
                command = ":SOUR:DEL 0";
                this.gpib.Device.Write(command);
                command = ":OUTP ON";
                ...
                rtn = this.gpib.Query(command);
                Thread.Sleep(100);

                string substring1 = (rtn.Substring(0, 10));
 
           
                //return double.Parse(rtn);
                return rtn;

                //int start = 0, length = rtn.Length;

                //Console.WriteLine(rtn.Substring(start - 1, length)); 
        ...
        public string Scan2()
        {
            ...
            this.gpib.Device.Write(command);
            command = ":SENS:FUNC 'CURR:DC'";
            this.gpib.Device.Write(command);
            command = ":SENS:CURR:PROT 0.1";
            this.gpib.Device.Write(command);
            command = ":SOUR:VOLT:MODE LIST";
            this.gpib.Device.Write(command);
            command = ":SOUR:LIST:VOLT 7,1,3,8,2";
            this.gpib.Device.Write(command);
            command = ":TRIG:COUN 5";
            this.gpib.Device.Write(command);
            command = ":SOUR:DEL 0.1";
            this.gpib.Device.Write(command);
            command = ":OUTP ON";
            this.gpib.Device.Write(command);
            command = ":READ?";
        ...
        //private string Scan(double startValue, double points, double stopValue, double measRange)
        ...
        //        command = ":RES:MODE AUTO";
        //        this.gpib.Device.Write(command);

        //        command = ":RES:RANG 3E3";
        //        this.gpib.Device.Write(command);

        //        command = ":SYST:RSEN OFF";
        //        this.gpib.Device.Write(command);

        //        //command = ":RES:OCOM OFF";
        //        //this.gpib.Device.Write(command);

        //        command = ":FORM:ELEM RES";
        //        this.gpib.Device.Write(command);

        //        //command = ":SENSe:VOLTage:PROTection 1";
        //        //this.gpib.Device.Write(command);

        //        //command = ":SENSe:CURRent:PROTection 1";
        //        //this.gpib.Device.Write(command);

        //        //command = ":SOURce:FUNCtion CURR";
        //        //this.gpib.Device.Write(command);
        //        command = ":OUTPut ON";
        //        this.gpib.Device.Write(command);
        //        command = ":READ?";
